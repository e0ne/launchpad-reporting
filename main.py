...
def bug_list(project_name, bug_type, milestone_name):
    ...
    bugs = lpdata.get_bugs(project_name,
                           LaunchpadData.BUG_STATUSES[bug_type],
                           milestone_name,
                           tags)
...
def bug_report_get_incoming_outgoing_data(project_name, milestone_name):
    data = ReleaseChart(lpdata,
                        project_name,
                        milestone_name).get_incoming_outgoing_data()
    return flask.json.dumps(data)


@app.route('/')
def main_page():
    return flask.redirect(flask.url_for("project_overview",
